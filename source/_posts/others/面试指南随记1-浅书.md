...
3、arguments改变参数的值不可以映射到我们的形参变量上

4、不允许使用arguments.callee和arguments.caller

5、使用call和apply改变this关键字，写的是null/undefined就不在是window了而是自己null/undefined本身

6、eval和arguments虽然不是关键字和保留字，但是也不可以定义变量
...
**将脚本放在底部。**``还是放在`head`中，用以保证在`js`加载前，能加载出正常显示的页面。``标签放在``前。
